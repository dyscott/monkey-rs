...
fn first(args: Vec<Object>) -> Result<Object> {
    ...
    match args[0] {
        ...
        _ => Err(anyhow!("argument to `first` must be ARRAY, got {}", args[0].type_name())),
    }
}
...
fn last(args: Vec<Object>) -> Result<Object> {
    ...
    match args[0] {
        ...
        _ => Err(anyhow!("argument to `last` must be ARRAY, got {}", args[0].type_name())),
    }
}
...
fn rest(args: Vec<Object>) -> Result<Object> {
    ...
    match args[0] {
        ...
        _ => Err(anyhow!("argument to `rest` must be ARRAY, got {}", args[0].type_name())),
    }
}

fn push(args: Vec<Object>) -> Result<Object> {
    if args.len() != 2 {
        return Err(anyhow!("wrong number of arguments. got={}, want=2", args.len()));
    }
